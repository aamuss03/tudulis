...
export default function TodoList() {
  ...
  const addTask = async (): Promise<void> => {
    const { value: formValues } = await Swal.fire({
      ...
      html:
        '<input id="swal-input1" class="swal2-input custom-input" placeholder="Nama kegiatan">' +
        '<input id="swal-input2" type="datetime-local" class="swal2-input custom-input">',
      focusConfirm: false,
  ...
  const editTask = async (task: Task) => {
    const { value: formValues } = await Swal.fire({
      title: "Edit Kegiatan",
      html:
        `<input id="swal-input1" class="swal2-input custom-input" value="${task.text}" placeholder="Nama kegiatan">` +
        `<input id="swal-input2" type="datetime-local" class="swal2-input custom-input" value="${task.deadline}">`,
      focusConfirm: false,
      showCancelButton: true,
            ...
            {sortedTasks.map((task) => {
              ...
              return (
                <motion.li
                  ...
                  className={`grid grid-cols-12 items-center gap-4 px-6 py-4 rounded-xl border ${rowColor}`}
                >
